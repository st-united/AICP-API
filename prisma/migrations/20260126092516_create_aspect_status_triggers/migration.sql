...
RETURNS TRIGGER AS $$
BEGIN
    -- Optimized: Update ALL affected aspects in a single statement instead of loop
    UPDATE "CompetencyAspect" ca
    SET status = CASE 
        -- Check Logic Inline for Speed:
        -- 1. Must have Assessment Method
        -- 2. Must have valid Pillar Link
    ...
    -- Optimization: Only update if status actually changes
    AND status != (
        CASE 
            WHEN EXISTS (SELECT 1 FROM "CompetencyAspectAssessmentMethod" WHERE competency_aspect_id = ca.id) 
                 AND EXISTS (

/*
  ...
  - Added the required column `domain_id` to the `Exam` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Exam" ADD COLUMN     "domain_id" UUID NOT NULL,
ADD COLUMN     "exam_status" "ExamStatus",
ADD COLUMN     "level_of_domain" "SFIALevel",
ALTER COLUMN "exam_set_id" SET NOT NULL;

-- AddForeignKey
ALTER TABLE "Exam" ADD CONSTRAINT "Exam_domain_id_fkey" FOREIGN KEY ("domain_id") REFERENCES "Domain"("id") ON DELETE CASCADE ON UPDATE CASCADE;
